      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          {listing.images.length > 0 ? (
              ...
              {listing.images.length > 1 && (
                <div className="flex gap-2 overflow-x-auto">
                  {listing.images.slice(1).map((img) => (
                    <div key={img.id} className="relative h-16 w-16 shrink-0 rounded-lg overflow-hidden bg-gray-100">
                      <Image
            ...
            <div className="h-80 rounded-xl bg-gray-100 flex items-center justify-center text-6xl">📦</div>
          )}
        </div>

        {/* Details */}
        <div>
          <span className="inline-block bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-1 rounded-full mb-3">
            {listing.categoryLabel}
          </span>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">{listing.title}</h1>
          <p className="text-3xl font-bold text-yellow-600 mb-4">${listing.price.toFixed(2)}</p>
