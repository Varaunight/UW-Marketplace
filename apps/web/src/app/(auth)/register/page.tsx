...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError('');
    if (!form.email.endsWith('@uwaterloo.ca')) { setError('Only @uwaterloo.ca email addresses are allowed.'); return; }
    if (form.password !== form.confirmPassword) { setError('Passwords do not match.'); return; }
    if (form.password.length < 8) { setError('Password must be at least 8 characters.'); return; }
    setLoading(true);
    try {
      await apiClient().post('/auth/register', { email: form.email, password: form.password, displayName: form.displayName });
      setSuccess(true);
    } catch (err) {
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="bg-surface border border-border rounded-2xl p-10 w-full max-w-md text-center">
          ...
          <p className="text-muted text-sm leading-relaxed">
            We sent a verification link to <strong className="text-fg">{form.email}</strong>. Click it to activate your account.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="bg-surface border border-border rounded-2xl p-8 w-full max-w-md">
        <div className="mb-6">
          ...
          <p className="text-muted text-sm mt-1">UW students only — @uwaterloo.ca required</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" required value={form.displayName} onChange={(e) => setForm({ ...form, displayName: e.target.value })} className={inputClass} placeholder="Jane Smith" />
          </div>
          <div>
